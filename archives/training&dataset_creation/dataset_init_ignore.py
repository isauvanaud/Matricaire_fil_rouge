...
# =========================
# FILTRAGE DES IMAGES SANS LABEL
# =========================
valid_images = []
ignored = 0
...
for img in images:
    if img.with_suffix(".txt").exists():
        ...
        print(f"⚠ Annotation manquante, image ignorée : {img.name}")
        ignored += 1

images = valid_images

if len(images) == 0:
    raise RuntimeError("Aucune image valide avec annotation")

print(f"{ignored} images ignorées (annotation manquante)")
